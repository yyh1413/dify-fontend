...
<template>
  <div class="container">
    <div class="container_top mb_15">
      <h4>冲刺规划</h4>
      <div>
        <el-form label-width="70px" :model="form" ref="formRef" size="small">
          <el-row :gutter="20">
            <el-col :span="5">
              <el-form-item
                label="名称"
            ...
            <el-col :span="9">
              <el-form-item label="起止时间" prop="date" class="no-item-style">
                <el-date-picker
                  v-model="form.date"
                  type="daterange"
                  range-separator="To"
    ...
    <div class="container_bottom">
      ...
      >
        <template v-slot:tableColumn>
          <el-table-column label="冲刺名" prop="sprintName">
            <template #default="scope">
              <span @click="handlePge(scope.row)">{{
                scope.row.sprintName
              }}</span>
            </template>
          </el-table-column>
          <el-table-column label="起始时间" prop="startDate" />
          <el-table-column label="结束时间" prop="endDate" />
          <el-table-column label="所属季度" prop="quarter" />
...
const del = (row: any) => {
  //删除
  ElMessageBox.confirm("此操作将删除本冲刺下任务，是否继续?", "提示", {
    type: "warning",
  }).then(async () => {
    const param = {
      dataId: row?.id,
